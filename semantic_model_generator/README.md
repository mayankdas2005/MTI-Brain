...
Copy `.env.example` to `.env` in the `semantic_model_generator/` directory and fill in the required values:
...
| File | Description |
|------|-------------|
| `output/lpp_semantic_model.yml` | R2RML mappings — 105 tables with ontology classes and column-level predicate mappings. Source of truth for table/column definitions passed to the pipeline. Generated by `generate_semantic_model.py`. |
| `output/lpp_semantic_model_with_descriptions.yml` | Same as above plus LLM-generated descriptions on tables and columns. Written by the `enrich` step. |
| `output/relationships.yml` | Extracted FK relationships in a flat format. Intermediate output from `generate_semantic_model.py`. |
...
| `output/join_key_profile.json` | Join key quality report (safe / caution / dangerous verdicts per join pair). Generated by `profile_join_keys.py`. |
...
| `generate_semantic_model.py` | Standalone Redshift schema extractor — generates `output/lpp_semantic_model.yml` |
| `profile_join_keys.py` | Standalone join key quality profiler — scores join pairs (safe / caution / dangerous) |
| `check_output.py` | Utility to inspect and summarise `output/join_key_profile.json` |
...
Reads the live Redshift `lpp` schema and writes `output/lpp_semantic_model.yml`. Run this first to bootstrap or refresh the source-of-truth YAML before running the pipeline.

```bash
python generate_semantic_model.py
```

Requires Redshift credentials in `.env`. Writes incrementally — one table at a time — so it can be safely interrupted and restarted.

---

### profile_join_keys.py

Statistical profiler for join key pairs. Collects join pairs from YAML relationships, Neo4j `JOINS_TO` edges, and `JoinPath` nodes, then scores each pair for join safety.

```bash
python profile_join_keys.py [--skip-neo4j] [--joins-file PATH] [--output PATH]
```

| Flag | Description |
|------|-------------|
| `--skip-neo4j` | Skip live Neo4j queries; use YAML relationships only |
| `--joins-file PATH` | Load additional join pairs from a pre-exported JSON file |
| `--output PATH` | Write profile to a custom path (default: `output/join_key_profile.json`) |

Output verdicts:
- **safe** — high overlap, low fanout risk
- **caution** — moderate overlap or multiplicity
- **dangerous** — low overlap or high join row explosion potential

---

### check_output.py

Prints a summary of `output/join_key_profile.json`: verdict distribution, fanout risk breakdown, and sample pairs per verdict.
