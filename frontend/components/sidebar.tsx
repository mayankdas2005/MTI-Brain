...
export function Sidebar({ forceExpanded = false, forceCollapsed = false }: { forceExpanded?: boolean; forceCollapsed?: boolean } = {}) {
  ...
  return (
    <div data-no-contrast className={cn("flex flex-col h-full bg-sidebar overflow-hidden shrink-0", !forceExpanded && "border-r border-sidebar-border transition-[width] duration-200", forceExpanded ? "w-full" : isOpen ? "w-[280px]" : "w-12")} data-onboarding="sidebar">
      ...
      {/* Chats navigation — dropdown with last 5 chats */}
      {isOpen ? (
        <div className="px-3 pb-1 shrink-0 sidebar-content-enter">
          <Collapsible open={chatsOpen} onOpenChange={setChatsOpen}>
            ...
            <CollapsibleContent>
              <div className="mt-1 space-y-[var(--density-list-gap)] px-1">
                {threads.length === 0 ? (
                  <p className="text-xs text-sidebar-foreground/50 px-2 py-2" suppressHydrationWarning>
      ...
      <div className={cn("border-t border-sidebar-border shrink-0 min-h-[3.5rem] flex flex-col justify-center", isOpen ? "px-3" : "items-center")} style={{ paddingTop: 'var(--density-pad-y)', paddingBottom: 'var(--density-pad-y)' }}>
        {!user ? (
          isOpen ? (
