...
def _build_data_summary(
    ...
    if n <= _NARRATIVE_SAMPLE_CAP:
        spread_sample = rows
    elif columns and columns[0] == "source":
        from collections import defaultdict
        by_source: dict = defaultdict(list)
        for r in rows:
            by_source[str(r[0])].append(r)
        per_source = max(3, _NARRATIVE_SAMPLE_CAP // max(1, len(by_source)))
        spread_sample = []
        for src_rows in by_source.values():
            m = len(src_rows)
            if m <= per_source:
                spread_sample.extend(src_rows)
            else:
                step = max(1, m // per_source)
                spread_sample.extend(src_rows[::step][:per_source])
    else:
        step = max(1, (n - 20) // 10)
        middle_indices = list(range(10, n - 10, step))[:10]
        spread_sample = (
            rows[:10]
            + [rows[i] for i in middle_indices]
            + rows[-10:]
        )
