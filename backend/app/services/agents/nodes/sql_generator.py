...
def _build_query_blueprint(
    ...
    if fx_rate_template_join:
        ...
        lines.append("")
        lines.append("  -- CTE 1: most recent available date per currency (within 14 days), AVG across rate_types")
        ...
        lines.append("              AND f2.rate_date     <= <date_ref>")
        lines.append("              AND f2.rate_date     >= DATEADD(DAY, -14, <date_ref>)")
        lines.append("          )")
        lines.append("    GROUP BY f.base_currency")
        lines.append("  ),")
        lines.append("")
        lines.append("  -- CTE 2: join source data to fx_latest and convert amount to USD")
        ...
        lines.append("  )")
        lines.append("")
        lines.append("Fill in: <date_ref> (reference date for the window, e.g. CURRENT_DATE or src.<date_col>),")
        lines.append("         <source_cte>, <currency_col>, <amount_col>.")
        ...
        lines.append("Do NOT add: ROW_NUMBER, PARTITION BY, rate_type filter, source filter, or CURRENT_DATE equality join.")
        lines.append("")
